...
<script type="text/javascript">
	...
	$(function() {
		$("#table").bootstrapTable({
			...
			onEditableSave : function(field,row,oldValue,event) {
				var td = $("#table tbody tr:eq("+row.rowId+") td:eq(1)");
				$(td).html(row.desc);
			},
			onPostBody : function() {
				var rows = $("#table").bootstrapTable("getData");
				for(var i = 0;i< rows.length;i++) {
					if(!isNaN(rows[i].desc)) {
						var td = $("#table tbody tr:eq("+rows[i].rowId+") td:eq(1)");
						$(td).html(rows[i].desc);
					}
				}
			},
...
<body>
	...
	<button class="btn btn-primary" onclick="printData()">打印</button>
