...
public class DownloadChapter extends EBookAdapter {
	...
	public static void main(String[] args) {
		try {
			ResultSet resultSet = JConnector.preparedStatement("select * from novel where id=36893").executeQuery();
			if (resultSet.next()) {
				...
				for(Entry entry : entries) {
					...
					service.submit(new Runnable() {
						...
						@Override
						public void run() {
							String content = dc.getContent(entry, url, FilterType.ID, "ccontent");
							if(!"".equals(content)) {
								try {
